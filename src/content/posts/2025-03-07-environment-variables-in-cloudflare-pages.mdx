...
but there is an issue. You cannot use environment variables in Pages. Using env vars in Static Assets is fairly straightforward, you decalre them in the `vars` scope in `wrangler.toml` (or `jsonc`) and you get access to them in the context passed on by the request.

But I was not interested in going via the Workers route. I wanted to deploy my SPA _AND_ to also use the environment variables.
...
Now when you run the project, and use the url via `import.meta.env.VITE_API_URL`, everything would work as expected since during development we're not replacing anything.
...
- In the _Build_ step, we're setting the variables we want to set for each branch.
...
In both of these methods, we're essentially replacing the variable names which we're using with the actual variables' content during the build step.
