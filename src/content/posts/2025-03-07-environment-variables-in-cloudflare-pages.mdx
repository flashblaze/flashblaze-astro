...
But there is an issue. You cannot use environment variables in Pages. Using env vars in Static Assets is fairly straightforward. You declare them in the `vars` scope in `wrangler.toml` (or `jsonc`) and you get access to them in the context passed on by the request.
