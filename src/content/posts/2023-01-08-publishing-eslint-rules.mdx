...
description: 'Walkthrough on how to publish ESLint rules to npm so that they can be reused in multiple projects'
date: '2023-01-08'
---
...
After installing the packages by running `pnpm i` from the root of the directory, the directory structure is like so:
...
To use the above base into `react` package, we use it like so
...
To use `@your-org/eslint-config-base` add it as a dependency. To do so, go into `packages/react` and run `pnpm add @your-org/eslint-config-base`. This will automatically install the package from the workspace and will be shown like so:
...
Now that the configs are created, next step would be to use them in a project and see whether they are working or not. To do so, create a Vite project with React template and link it. You can check the commands [here](https://eslint.org/docs/latest/extend/shareable-configs#publishing-a-shareable-config) or follow the below commands.
