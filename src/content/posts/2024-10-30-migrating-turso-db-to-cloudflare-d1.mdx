...
D1 as well as drizzle-kit requires the project to have a `wrangler.toml` file. As such here is mine:
...
Based on their docs it seemed like drizzle expects the argument to be of type `D1Database`. Since `env.d.ts` file is being used throughout the application for environment related stuff, I updated it like so:
...
I wanted to take a dump of my data. After searching of the same, I found out about the `dump` command. And so I ran `turso db shell map-viz ".dump" > map-viz.sql`. `map-viz` being the name of my database. However, every single time, the `map-viz.sql` was being created with empty content. I'm not sure what was wrong since I was correctly executing the command and the data was also present. So I finally resorted to manually exporting the data in the CSV format.
